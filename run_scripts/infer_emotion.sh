...
OUTPUT_FILE="${HOME_PATH}/output/emotion_dataset/${DATASET}_iter1.jsonl"
CONFIG_JSON="${HOME_PATH}/Facial-R1/src/open-r1-multimodal/src/open_r1/configs/config.json"

# 设置工作目录和Python路径
cd $HOME_PATH/Facial-R1/src/inference
...
# 使用torchrun执行分布式推理
torchrun --nproc_per_node=4 --master_port="12346" inference_replace_desc.py \
  --main_rank 0 \
